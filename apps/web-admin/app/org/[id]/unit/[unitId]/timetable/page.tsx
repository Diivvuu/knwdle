...
import {
  TimetableEntry,
  fetchUnitTimetable,
  selectUnitTimetableEntries,
  selectUnitTimetableStatus,
  selectUnitTimetableError,
} from '@workspace/state';

const DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

export default function UnitTimetablePage() {
  const { id: orgId, unitId } = useParams() as { id: string; unitId: string };
  const dispatch = useDispatch<AppDispatch>();
  const entries = useSelector((s: RootState) =>
    selectUnitTimetableEntries(s, unitId)
  );
  const status = useSelector(selectUnitTimetableStatus);
  const error = useSelector(selectUnitTimetableError);

  const [view, setView] = useState<'list' | 'editor'>('list');

  useEffect(() => {
    if (orgId && unitId) {
      dispatch(fetchUnitTimetable({ orgId, unitId }));
    }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-xs text-muted-foreground">
        Timetable applies to all students in this unit. Attendance will be
                      ...
                      >
                        ...
                        {(match.room || match.mode) && (
                          <div className="text-[11px] text-muted-foreground">
                            {[match.room, match.mode].filter(Boolean).join(' | ')}
                          </div>
                        )}
                      </div>
                    ))
