...
Knwdle is an open-source, multi-tenant education & organisation platform. It gives schools, coaching centres, universities, and training orgs one place to manage people, audiences (departments/classes), attendance, announcements, assessments, fees, and a “Connect” portal for students/parents.
...
- `apps/api` – Express + Prisma API, JWT auth, multi-tenant org/audience model, S3 uploads (Localstack in dev). API map: `apps/api/src/api-map.md`.
...
Implemented domains: auth/session, invites, orgs & audiences, roles/permissions, uploads, notifications, org + audience dashboards, attendance, Connect dashboard. Planned/next domains: assignments, tests/results, fees, timetable/calendar, announcements, notes/content, messaging, analytics, audit, achievements. See `apps/api/src/api-map.md` for current endpoints.
